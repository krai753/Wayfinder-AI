...
# ── Retry counter + auto-CS escalation ────────────────────────
# If user gives invalid input on a field 5+ times, offer CS escalation


RETRY_THRESHOLD = 5


def _increment_retry(session_id: str) -> int:
    """Increment retry counter for a session. Returns current count."""
    from database import update_session as update_db_session, get_session
...
def _reset_retry(session_id: str):
    """Reset retry counter after valid input."""
    from database import update_session as update_db_session
    update_db_session(session_id, retry_count=0)


def _check_retry_escalation(session_id: str, user_lang: str = "") -> VoiceCommandResponse | None:
    """
    Check if retry count has exceeded threshold and return escalation response if so.
    Returns None if under threshold (continue normally).
    """
    ...
    # Reset counter and create CS ticket
    _reset_retry(session_id)
    ...
    create_cs_ticket(ticket_id, session_id=session_id, user_name=user_name, issue="User struggling — repeated invalid input")

    response = _translate_response(
        "I'm having trouble understanding your input. Would you like me to connect you to a customer service agent who can assist you directly? Please say yes or no.",
        user_lang,
    )
    return VoiceCommandResponse(
        intent="cs_escalation",
        parameters={"reason": "retry_exceeded", "ticket_id": ticket_id, "session_id": session_id, "confirm_needed": True},
...
async def _handle_search_flights(params: dict, response_text: str, session_id: str | None) -> VoiceCommandResponse:
    ...
    if not get_airport(origin):
        if sid_for_retry:
            _increment_retry(sid_for_retry)
            esc = _check_retry_escalation(sid_for_retry, user_lang)
            if esc:
                return esc
        return VoiceCommandResponse(
...
async def _handle_provide_name(params: dict, response_text: str, session_id: str | None) -> VoiceCommandResponse:
    ...
    if not name:
        if sid:
            _increment_retry(sid)
            esc = _check_retry_escalation(sid, user_lang)
            if esc:
                return esc
        return VoiceCommandResponse(
            intent="provide_name",
